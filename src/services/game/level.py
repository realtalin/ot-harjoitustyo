...
class Level:
    ...
    def __init__(self, size, visual_grid_size, init_time=0):
        """The constructor. Calls init_cells and generate_correct_cells.

        Args:
            size (int): Width/height of the level
            visual_grid_size (int): Width/height of the window
        ...
        self._correct_cells = pygame.sprite.Group()
        self._cell_backgrounds = pygame.sprite.Group()
        self.all_sprites = pygame.sprite.Group()

        self._init_cells(visual_grid_size)
        self._generate_correct_cells(self.size*2)

    @staticmethod
    def create_level(size, visual_grid_size, _init_time):
        """Static method for creating a new level.

        Args:
            size (int): Width/height of the level in cells
        ...
        Returns:
            A new level object
        """
        return Level(size, visual_grid_size, _init_time)

    def _calculate_cell_size(self, visual_grid_size):
        ...
        """
        return visual_grid_size / self.size * 0.9

    def _calculate_cell_coordinates(self, visual_grid_size, cell_size, row, column):
        ...
        Args:
            visual_grid_size (int): Widht/height of the window
        ...
        """
        cell_gap = 1/0.9
        level_edge = (visual_grid_size / self.size) * 0.05
    ...
    def _init_cells(self, visual_grid_size):
        ...
        for row in range(self.size):
            ...
            for column in range(self.size):
                ...
                cell_size = self._calculate_cell_size(visual_grid_size)
                cell_coordinates = self._calculate_cell_coordinates(
                    visual_grid_size, cell_size, row, column)

                cell = Cell(cell_size, cell_coordinates)
                self._cells.add(cell)

                cell_background = CellBackground(cell_size, cell_coordinates)
                self._cell_backgrounds.add(cell_background)

        self.all_sprites.add(self._cells, self._cell_backgrounds)
    ...
    def _correct_showing_time_left(self, time):
        ...
        """
        if 2200 - (time - self._init_time) > 0:
            return True
        return False
    ...
    def click_on_incorrect_cell(self, mouse_position, time):
        ...
        for cell in self._cells:
            if cell.rect.collidepoint(mouse_position):
                cell.click()
                if cell not in self._correct_cells:
                    return True
        return False
    ...
    def all_correct_clicked(self):
        ...
        """
        all_correct = True
        for cell in self._correct_cells:
    ...
    def update_state(self, time):
        ...
        Args:
            time (int): The current time
        """
        self._update_cell_visibility(time)
        self._cells.update()

    def _update_cell_visibility(self, time):
